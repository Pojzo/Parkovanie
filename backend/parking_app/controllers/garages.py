...
from parking_app.schema.requests import CreateGarageRequest
...
async def _delete_garage(garage: GarageModel, db: Connection):
    async with db.cursor() as cursor:
        await cursor.execute("DELETE FROM ParkingGarage WHERE garage_id = %s", (garage.garage_id,))
        await db.commit()
...
async def handle_create_garage(rq: CreateGarageRequest, db: Connection):
    if await _get_garage_by_name(rq.name, db) is not None:
        raise HTTPException(status_code=409, detail="Garage with this name already exists")
...
async def handle_get_all_garages(db: Connection):
    ...
    return SuccessResponse[list[GarageModel]](data=all_garages, message="All garages retrieved")
